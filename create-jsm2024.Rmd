...
knit: litr::render
params:
  package_name: "jsm2024" # <-- change this to your package name
  package_parent_dir: "." # <-- relative to this file's location
---

<!-- This Rmd file contains all the code needed to define an R package.  Press "Knit" in RStudio or more generally run `rmarkdown::render("create-jsm2024.Rmd")` to generate the R package.  Remember that when you want to modify anything about the R package, you should modify this document rather than the package that is outputted.
-->
...
#' @source From the JSM 2024 program.
"words"
```
...
1. Metadata from all arxiv papers in stat.XX from January 1, 2013 to July 15, 2024 (using the [arxiv bulk metadata access OAI protocol](https://arxiv.org/help/oa/index)).  There were 111,459 such papers.

2. Semantic Scholar (S2)'s bulk data sets (on papers, authors, and citations), in which we started with about 200M papers and filtered down to about 350K papers (written by about 390K authors) by restricting attention to papers in statistics journals or having the arxiv stat.XX tag. Note: Due to a technical challenge with the 2024 data, we used what had been computed for 2023.

3. The JSM 2024 program itself, along with the list of authors that was scraped from JSM 2023 and JSM 2022.  There are 12,628 names in total.
...
```{r}
#' JSM author names
#' @format A character vector of 12,628 names from past and present JSMs
#' @source This data was from the JSM 2022-2024 programs.
"authors"
```
...
```{r}
authors2024 <- jsm$authors_thisyear
```

We will want to have precomputed recommendations available for everyone in `authors`.  This combines everyone who has appeared on the JSM program 2022-2024.  However, we want these recommendations to be about only those people who are actually giving talks in 2024.  (E.g., it's not useful to see the name of a coauthor of mine if they aren't presenting at JSM this year!)

```{r}
#' JSM 2024 author names
#' @format A character vector of 5,816 JSM 2024 author names
#' @source This data was from the JSM 2024 program.
"authors2024"
```
...
desc::desc_set(
  "URL",
  "https://github.com/jacobbien/jsm2024-project/tree/main/jsm2024"
  )
```
...
```{r}
add_hex_sticker("../source-files/jsm2024-hex.png")
```
