...
export_calendar_to_ics <- function(schedule, file = NULL) {
  ...
  cal$event$DESCRIPTION <- stringr::str_glue_data(
    schedule, 
    "Authors: {author}\\nSession: <a href='https://ww3.aievolution.com/JSMAnnual2024/Events/viewEv?ev={session_number}' target='_blank'>{session_title}</a> ({session_type})\\nChair: {chair}")
  cal$event %>% 
